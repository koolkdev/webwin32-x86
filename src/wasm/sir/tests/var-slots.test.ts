...
import { assignSirExprVarSlots } from "../var-slots.js";
...
test("assignSirExprVarSlots reuses slots after last use", () => {
  const slots = assignSirExprVarSlots([
...
test("assignSirExprVarSlots can reuse a last-use input slot for a let destination", () => {
  const slots = assignSirExprVarSlots([
    { op: "let32", dst: v(0), value: c32(1) },
...
test("assignSirExprVarSlots keeps overlapping values in separate slots", () => {
  const slots = assignSirExprVarSlots([
    { op: "let32", dst: v(0), value: c32(1) },
    { op: "let32", dst: v(1), value: c32(2) },
