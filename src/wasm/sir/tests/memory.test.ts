...
import {
  emitWasmSirLoadGuestU32,
  emitWasmSirLoadGuestU32FromStack,
  emitWasmSirStoreGuestU32
} from "../memory.js";
...
function encodeGuestLoadModule(mode: "local" | "stack"): Uint8Array<ArrayBuffer> {
  ...
  body.block();
  if (mode === "local") {
    emitWasmSirLoadGuestU32({ body, exit }, 0);
  } else {
    body.localGet(0);
    emitWasmSirLoadGuestU32FromStack({ body, exit }, 0);
  }
  emitWasmSirExitFromI32Stack(body, exit, ExitReason.FALLTHROUGH);
  body.endBlock();
...
function encodeGuestStoreModule(): Uint8Array<ArrayBuffer> {
  ...
  body.block();
  emitWasmSirStoreGuestU32({ body, exit }, 0, 1);
  body.i32Const(0);
  emitWasmSirExitFromI32Stack(body, exit, ExitReason.FALLTHROUGH);
  body.endBlock();
  body.localGet(exitLocal).end();
