...
export function analyzeJitBlockState(
  ...
  for (let instructionIndex = 0; instructionIndex < block.instructions.length; instructionIndex += 1) {
    ...
    for (let opIndex = 0; opIndex < instruction.ir.length; opIndex += 1) {
      ...
      const faultReason = jitPreInstructionExitReasonAt(analysis.context.effects, instructionIndex, opIndex);
  ...
  function recordPostInstructionExits(
    ...
    const exitReasons = jitPostInstructionExitReasonsAt(analysis.context.effects, instructionIndex, opIndex);
